...
{	
    ...
    case 6:
        {
            node->get_parameter_or("target_joint_1", tar_joint1, {1.570796, -1.570796, -1.047198, 2.967060, 2.792527, -3.124139});
            node->get_parameter_or("target_joint_2", tar_joint2, {0, 0, 0, 0, 0, 0});
    ...
    while (rclcpp::ok())
    {
        ...
        RCLCPP_INFO(node->get_logger(), "move to tar_joint1");
        
        joint_plan_req->target = tar_joint2;
        call_request(joint_plan_client_, joint_plan_req);
        call_request(exec_plan_client_, exec_plan_req);
        RCLCPP_INFO(node->get_logger(), "move to tar_joint2");
        
        joint_plan_req->target = tar_joint3;
        call_request(joint_plan_client_, joint_plan_req);
        call_request(exec_plan_client_, exec_plan_req);
        RCLCPP_INFO(node->get_logger(), "move to tar_joint3");
        
        joint_plan_req->target = tar_joint4;
        call_request(joint_plan_client_, joint_plan_req);
        call_request(exec_plan_client_, exec_plan_req);
        RCLCPP_INFO(node->get_logger(), "move to tar_joint4");
